    ...
    io.on('connection', (socket) => {
        ...
        socket.on('player-action', (data) => {
            try {
                ...
                if (state.phase === 'finished' && state.lastHandResult) {
                    ...
                    const persisted = gameHistoryService_1.gameHistoryService.recordHandFinished(state.id, {
        ...
        socket.on('end-game', async () => {
            try {
                ...
                gameHistoryService_1.gameHistoryService.markGameEnded(state.id, endedAt);
                gameHistoryService_1.gameHistoryService.closeAllOpenSessionsForGame(state.id, endedAt);
