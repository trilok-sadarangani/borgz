...
describe('Game Rules', () => {
    describe('validateAction', () => {
        ...
        describe('fold', () => {
            it('should allow fold when player is active', () => {
                ...
                const result = (0, rules_1.validateAction)(player, 'fold', undefined, 0, 20);
        ...
        describe('check', () => {
            it('should allow check when no bet', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'check', undefined, 0, 20);
                expect(result.valid).toBe(true);
            });
            it('should not allow check when there is a bet', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'check', undefined, 50, 20);
                expect(result.valid).toBe(false);
        ...
        describe('call', () => {
            it('should allow call when there is a bet', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'call', undefined, 50, 20);
                expect(result.valid).toBe(true);
            });
            it('should not allow call when no bet', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'call', undefined, 0, 20);
                expect(result.valid).toBe(false);
        ...
        describe('raise', () => {
            it('should allow valid raise', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'raise', 100, 50, 20);
                expect(result.valid).toBe(true);
            });
            it('should require raise amount', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'raise', undefined, 50, 20);
                expect(result.valid).toBe(false);
                expect(result.error).toContain('Raise amount required');
            });
            it('should not allow raise lower than current bet', () => {
                const player = createPlayer();
                const result = (0, rules_1.validateAction)(player, 'raise', 40, 50, 20);
                expect(result.valid).toBe(false);
            ...
            it('should enforce minimum raise', () => {
                ...
                const result = (0, rules_1.validateAction)(player, 'raise', 60, 50, 70);
                expect(result.valid).toBe(false);
        ...
        describe('all-in', () => {
            it('should allow all-in when player has chips', () => {
                ...
                const result = (0, rules_1.validateAction)(player, 'all-in', undefined, 50, 20);
                expect(result.valid).toBe(true);
            });
    ...
    describe('processAction', () => {
        ...
        it('should process fold correctly', () => {
            const player = createPlayer();
            const result = (0, rules_1.processAction)(player, 'fold', undefined, 50, 20);
        ...
        it('should process check correctly', () => {
            const player = createPlayer();
            const result = (0, rules_1.processAction)(player, 'check', undefined, 0, 20);
            expect(result.newBet).toBe(0);
            expect(result.chipsCommitted).toBe(0);
            expect(result.isAllIn).toBe(false);
        });
        it('should process call correctly', () => {
            const player = createPlayer();
            const result = (0, rules_1.processAction)(player, 'call', undefined, 50, 20);
        ...
        it('should process raise correctly', () => {
            const player = createPlayer();
            const result = (0, rules_1.processAction)(player, 'raise', 100, 50, 20);
        ...
        it('should process all-in raise', () => {
            ...
            const result = (0, rules_1.processAction)(player, 'raise', 200, 50, 20);
        ...
        it('should process all-in action', () => {
            const player = createPlayer({ stack: 500 });
            const result = (0, rules_1.processAction)(player, 'all-in', undefined, 50, 20);
    ...
    describe('calculateMinRaise', () => {
        it('should calculate minimum raise when no previous raise', () => {
            const minRaise = (0, rules_1.calculateMinRaise)(50, 0, 20);
            expect(minRaise).toBe(70); // 50 + 20
        });
        it('should calculate minimum raise based on last raise', () => {
            const minRaise = (0, rules_1.calculateMinRaise)(50, 30, 20);
    ...
    describe('getNextActivePlayer', () => {
        ...
        it('should return next active player', () => {
            ...
            const next = (0, rules_1.getNextActivePlayer)(players, 0, 0);
            expect(next).toBe(1);
        });
        ...
        it('should wrap around to beginning', () => {
            ...
            const next = (0, rules_1.getNextActivePlayer)(players, 2, 0);
            expect(next).toBe(0);
        });
