...
const texasHoldem_1 = require("../variants/texasHoldem");
describe('GameEngine Snapshot', () => {
    ...
    beforeEach(() => {
        ...
        game = new texasHoldem_1.TexasHoldem(settings, 'TEST01');
    ...
    describe('fromSnapshot()', () => {
        it('should restore waiting game state', () => {
            ...
            const restoredGame = engine_1.GameEngine.fromSnapshot(originalSnapshot);
        ...
        it('should restore game and allow continued play', () => {
            ...
            game.processPlayerAction(activePlayer.id, 'call');
            const snapshot = game.getSnapshot();
            const restoredGame = engine_1.GameEngine.fromSnapshot(snapshot);
    ...
    describe('TexasHoldem.fromSnapshot()', () => {
        it('should return a TexasHoldem instance', () => {
            game.startGame(player1.id);
            const snapshot = game.getSnapshot();
            const restored = texasHoldem_1.TexasHoldem.fromSnapshot(snapshot);
            expect(restored).toBeInstanceOf(texasHoldem_1.TexasHoldem);
            expect(restored).toBeInstanceOf(engine_1.GameEngine);
        });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle all-in situation', () => {
            ...
            const lowStackSettings = { ...(0, texasHoldem_1.createDefaultTexasHoldemSettings)(), startingStack: 50 };
            const lowStackGame = new texasHoldem_1.TexasHoldem(lowStackSettings, 'ALLIN1');
    ...
    describe('Complex Game State Snapshots', () => {
        ...
        it('should handle game with different stack sizes', () => {
            ...
            const settings = { ...(0, texasHoldem_1.createDefaultTexasHoldemSettings)(), startingStack: 5000 };
            const customGame = new texasHoldem_1.TexasHoldem(settings, 'STACK1');
    ...
    describe('Multiple Snapshot/Restore Cycles', () => {
        it('should handle multiple snapshot/restore cycles', () => {
            ...
            let restored = texasHoldem_1.TexasHoldem.fromSnapshot(snapshot);
            ...
            restored = texasHoldem_1.TexasHoldem.fromSnapshot(snapshot);
            // Make another action
        ...
        it('should maintain consistency through 5 snapshot cycles', () => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                currentGame = texasHoldem_1.TexasHoldem.fromSnapshot(snapshot);
            }
            // Game should still be in a valid state
            const finalState = currentGame.getState();
            expect(finalState.players).toHaveLength(2);
            expect(['pre-flop', 'flop', 'turn', 'river', 'showdown', 'finished']).toContain(finalState.phase);
        });
    });
    describe('Settings Preservation', () => {
        it('should preserve game settings through snapshot', () => {
            const customSettings = {
                ...(0, texasHoldem_1.createDefaultTexasHoldemSettings)(),
                smallBlind: 25,
                bigBlind: 50,
                maxPlayers: 6,
                turnTimerSeconds: 30,
            };
            const customGame = new texasHoldem_1.TexasHoldem(customSettings, 'SETT01');
            customGame.addPlayer(player1.id, player1.name);
            customGame.addPlayer(player2.id, player2.name);
            const snapshot = customGame.getSnapshot();
            const restored = texasHoldem_1.TexasHoldem.fromSnapshot(snapshot);
