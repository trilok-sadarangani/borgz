...
const handEvaluation_1 = require("./utils/handEvaluation");
...
class GameEngine {
    ...
    findNextIndexWithChips(fromIndex) {
    ...
    findNextCanAct(fromIndex) {
        ...
        for (let i = 0; i < n; i += 1) {
            const idx = (fromIndex + i) % n;
            const p = this.state.players[idx];
            if (p.isActive && !p.hasFolded && !p.isAllIn && p.stack > 0)
                return idx;
        }
        return -1;
    }
     ...
     */
    initializeGame(settings, gameCode) {
        return {
            id: (0, uuid_1.v4)(),
            code: gameCode,
    ...
    startGame(requesterId) {
        ...
        // Initialize deck
        this.deck = (0, cards_1.shuffleDeck)((0, cards_1.createDeck)());
        // Set positions
     ...
     */
    processPlayerAction(playerId, action, amount) {
        const player = this.state.players.find((p) => p.id === playerId);
        if (!player) {
            throw new Error('Player not found');
        }
        ...
        const minRaise = (0, rules_1.calculateMinRaise)(this.state.currentBet, this.lastRaise, this.state.settings.bigBlind);
        // Validate action
        const validation = (0, rules_1.validateAction)(player, action, amount, this.state.currentBet, minRaise);
        if (!validation.valid) {
            throw new Error(validation.error);
        }
        // Process action
        const result = (0, rules_1.processAction)(player, action, amount, this.state.currentBet, minRaise);
        // Update player state
        ...
        const nextIndex = (0, rules_1.getNextActivePlayer)(this.state.players, this.state.activePlayerIndex, this.state.dealerPosition);
        ...
        const allMatched = activePlayers.every((p) => p.currentBet === this.state.currentBet || p.isAllIn || p.stack === 0);
     ...
     */
    fastForwardIfNoFurtherBetting() {
        const canActCount = this.state.players.filter((p) => p.isActive && !p.hasFolded && !p.isAllIn && p.stack > 0).length;
        if (canActCount >= 2)
            return false;
     ...
     */
    dealFlop() {
        const { cards, remainingDeck } = (0, cards_1.dealCards)(this.deck, 3);
     ...
     */
    dealTurn() {
        const { card, remainingDeck } = (0, cards_1.dealCard)(this.deck);
        this.deck = remainingDeck;
    ...
    dealHoleCards() {
        this.state.players.forEach((player) => {
            ...
            const { card: card1, remainingDeck: deck1 } = (0, cards_1.dealCard)(this.deck);
            this.deck = deck1;
            const { card: card2, remainingDeck: deck2 } = (0, cards_1.dealCard)(this.deck);
            this.deck = deck2;
    ...
    postBlinds() {
        ...
        const smallBlindAmount = Math.min(this.state.settings.smallBlind, smallBlindPlayer.stack);
        const bigBlindAmount = Math.min(this.state.settings.bigBlind, bigBlindPlayer.stack);
        const ts = Date.now();
    ...
    postAntes() {
        ...
        if (!ante || ante.type === 'none' || ante.amount <= 0)
            return;
     ...
     */
    determineWinners() {
        const activePlayers = this.state.players.filter((p) => !p.hasFolded && p.isActive);
        ...
        const handResults = activePlayers.map((player) => ({
            player,
            result: (0, handEvaluation_1.evaluateHand)(player.cards, this.state.communityCards),
        }));
        // Sort by hand strength (best first)
        handResults.sort((a, b) => (0, handEvaluation_1.compareHands)(b.result, a.result));
        // Find all players with the best hand
        const bestHand = handResults[0].result;
        const winners = handResults.filter((hr) => (0, handEvaluation_1.compareHands)(hr.result, bestHand) === 0);
    ...
    getPublicState() {
        ...
        state.players = state.players.map((p) => ({ ...p, cards: undefined }));
        return state;
    }
    /**
