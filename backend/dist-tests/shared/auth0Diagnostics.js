...
exports.explainAuth0Failure = explainAuth0Failure;
function validateAuth0Config(config, ctx) {
    const issues = [];
    if (!config.domain)
        issues.push('Missing Auth0 domain.');
    if (!config.clientId)
        issues.push('Missing Auth0 client id.');
 ...
 */
function explainAuth0Failure(response, config, ctx) {
    if (!response)
        return null;
    ...
    const callbackMismatch = /callback url mismatch/i.test(desc) ||
    ...
    const serviceNotFound = /service not found/i.test(desc) ||
