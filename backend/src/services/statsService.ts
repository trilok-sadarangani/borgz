import { gameHistoryService, GameHistoryService, StoredHand } from './gameHistoryService';
...
export type PlayerStatsSummary = {
  ...
  totalWinnings: number; // chips (net)
  bb100: number | null;
  ...
  // Postflop
  aggressionFactor: number; // (bet+raise) / call, approximated as (raises+all-in-raises)/calls
  cbetPercentage: number; // Flop c-bet % for preflop aggressor
};
...
export type VsOpponentSummary = {
  ...
  showdownsWon: number;
  wsdVsOpponent: number; // showdownsWon/showdownsTogether * 100
};
...
function getNonBlindPositionLabels(n: number): PreflopPosition[] {
  ...
  switch (n) {
    ...
    default:
      ...
      const out: PreflopPosition[] = ['UTG'];
      for (let i = 1; i < count - 1; i += 1) out.push(`UTG+${i}` as PreflopPosition);
      out.push('BTN');
      return out;
  }
}

function computePlayerPreflopPosition(hand: StoredHand, playerId: string): PreflopPosition | null {
  const t = hand.table;
  if (!t) return null;
  ...
  if (n === 2) {
    ...
    if (seatIdx === dealerPosition) return 'BTN/SB';
    if (seatIdx === bigBlindPosition) return 'BB';
    // Defensive fallback
    if (seatIdx === smallBlindPosition) return 'BTN/SB';
    return null;
  }

  if (seatIdx === smallBlindPosition) return 'SB';
  if (seatIdx === bigBlindPosition) return 'BB';
  if (seatIdx === dealerPosition) return 'BTN';
...
function computePreflopOpportunitiesAndActions(
  ...
  for (const a of actions) {
    ...
    if (isForced) {
      if (typeof betAfter === 'number') currentBet = Math.max(currentBet, betAfter);
      continue;
    }
    ...
    if (isPlayer) {
      ...
      if (!openEvaluated) {
        ...
        if (openOpp && isRaise) openRaise = true;
      }
      ...
      if (!fourBetEvaluated && raiseCount === 2) {
        ...
        fourBetOpp = true;
        if (isRaise) fourBet = true;
      }
    } else if (!openEvaluated) {
      // Only track "prior voluntary money" until the player's first action (for RFI).
      if (a.action === 'call' || a.action === 'raise' || a.action === 'all-in') priorVoluntaryMoney = true;
    }
...
function parseDepthBucket(bb: number): DepthBucket {
  if (bb < 50) return '0-50';
  if (bb < 100) return '50-100';
  if (bb < 150) return '100-150';
  if (bb < 500) return '150-500';
  return '500+';
}

function computeEffectiveStackBbForPlayer(
  hand: StoredHand,
  settings: GameSettings,
  playerId: string
): number | null {
  ...
  if (typeof playerStack !== 'number') return null;
...
function getNetWinningsForHand(hand: StoredHand, playerId: string): number | null {
...
function computePreflopFlags(hand: StoredHand, playerId: string): { vpip: boolean; pfr: boolean; threeBet: boolean } {
  const actions = (hand.actions || [])
    .filter((a) => a.phase === 'pre-flop')
    .slice()
    .sort((a, b) => a.timestamp - b.timestamp);
...
function didPlayerFold(hand: StoredHand, playerId: string): boolean {
  return (hand.actions || []).some((a) => a.playerId === playerId && a.action === 'fold');
}

function sawShowdown(hand: StoredHand, playerId: string): boolean {
  if (hand.reason !== 'showdown') return false;
  // Approximation: if player folded at any point, they didn't see showdown.
  return !didPlayerFold(hand, playerId);
}

function wonAtShowdown(hand: StoredHand, playerId: string): boolean {
  if (!sawShowdown(hand, playerId)) return false;
  return hand.winners.some((w) => w.playerId === playerId);
}

function getPreflopAggressor(hand: StoredHand): string | null {
  const actions = (hand.actions || [])
    .filter((a) => a.phase === 'pre-flop')
    .slice()
    .sort((a, b) => a.timestamp - b.timestamp);
  ...
  for (const a of actions) {
    const isForced = a.action === 'post-blind' || a.action === 'post-ante';
    if (isForced) {
      if (typeof a.currentBetAfter === 'number') currentBet = Math.max(currentBet, a.currentBetAfter);
      continue;
    }
...
function computeCbetOnFlop(hand: StoredHand): { opp: boolean; made: boolean } {
  // Only consider hands that reached flop.
  const hasFlop = (hand.communityCards || []).length >= 3;
  if (!hasFlop) return { opp: false, made: false };

  const aggressor = getPreflopAggressor(hand);
  if (!aggressor) return { opp: false, made: false };

  // If aggressor folded before flop, no opportunity.
  const foldedPreflop = (hand.actions || []).some(
    (a) => a.phase === 'pre-flop' && a.playerId === aggressor && a.action === 'fold'
  );
  if (foldedPreflop) return { opp: false, made: false };
  ...
  if (!firstBet) return { opp: true, made: false };
  return { opp: true, made: firstBet.playerId === aggressor };
}

function computeAggressionCounts(hand: StoredHand, playerId: string): { aggressive: number; calls: number } {
  ...
  for (const a of actions) {
    if (a.action === 'call') calls += 1;
    if (a.action === 'raise') aggressive += 1;
    if (a.action === 'all-in') {
      // Count as aggressive only if it sets/raises the table bet.
      if ((a.currentBetAfter || 0) > 0) aggressive += 1;
    }
  }
...
export class PlayerStatsService {
  ...
  getMyStats(
    ...
    const gamesMap = new Map<string, GameInRange>();
    for (const c of contextsInRangeForGames) {
      if (query.clubId && c.clubId !== query.clubId) continue;
      if (query.variant && c.variant !== query.variant) continue;
      if (query.depthBucket) {
        const bb = computeEffectiveStackBbForPlayer(c.hand, c.settings, playerId);
        if (bb === null) continue; // no snapshot -> skip from bucketed views
        if (parseDepthBucket(bb) !== query.depthBucket) continue;
      }
    ...
    const fullyFiltered = filtered.filter((c) => {
      if (query.clubId && c.clubId !== query.clubId) return false;
      if (query.variant && c.variant !== query.variant) return false;
      if (query.gameId && c.gameId !== query.gameId) return false;
      if (query.code && c.code.toUpperCase() !== query.code.toUpperCase()) return false;
      if (query.depthBucket) {
        const bb = computeEffectiveStackBbForPlayer(c.hand, c.settings, playerId);
        if (bb === null) return false;
        if (parseDepthBucket(bb) !== query.depthBucket) return false;
      }
      return true;
    });
    ...
    const preflopByPos = new Map<PreflopPosition, PreflopPositionStats>();

    for (const c of fullyFiltered) {
      const hand = c.hand;
      totalHands += 1;

      if (hand.winners.some((w) => w.playerId === playerId)) handsWon += 1;

      const net = getNetWinningsForHand(hand, playerId);
      if (typeof net === 'number') totalWinnings += net;
      ...
      if (hasPreflopPhases) {
        ...
        if (flags.vpip) vpipCount += 1;
        if (flags.pfr) pfrCount += 1;
        if (flags.threeBet) threeBetCount += 1;
      ...
      if (pos && hasPreflopPhases) {
        ...
        if (pf.hadDecision) {
          ...
          prev.hands += 1;
          if (pf.pfr) prev.pfrCount += 1;
          if (pf.openOpp) prev.openOpps += 1;
          if (pf.openRaise) prev.openRaiseCount += 1;
          if (pf.threeBetOpp) prev.threeBetOpps += 1;
          if (pf.threeBet) prev.threeBetCount += 1;
          if (pf.fourBetOpp) prev.fourBetOpps += 1;
          if (pf.fourBet) prev.fourBetCount += 1;
      ...
      if (sawShowdown(hand, playerId)) {
        showdownHands += 1;
        wtsdCount += 1;
        if (wonAtShowdown(hand, playerId)) wsdCount += 1;
      }
      ...
      if (aggressor === playerId) {
        ...
        if (cbet.opp) cbetOpp += 1;
        if (cbet.made) cbetMade += 1;
      }
    }
    ...
    const positionOrder: PreflopPosition[] = [
      'UTG',
      'UTG+1',
      'UTG+2',
      'LJ',
    ...
    const preflop: PreflopCharts = {
      byPosition: Array.from(preflopByPos.values()).sort(
    ...
    const oppMap = new Map<
      string,
      { hands: number; winnings: number; bbSum: number; bbHands: number; sdTogether: number; sdWon: number }
    >();

    for (const c of fullyFiltered) {
      const stacks = c.hand.stacksStartByPlayerId;
      if (!stacks) continue;
      if (typeof stacks[playerId] !== 'number') continue;
      ...
      for (const oppId of opponentIds) {
        const e =
          oppMap.get(oppId) || { hands: 0, winnings: 0, bbSum: 0, bbHands: 0, sdTogether: 0, sdWon: 0 };
        e.hands += 1;
        if (typeof net === 'number') e.winnings += net;
        if (typeof bb === 'number') {
          e.bbSum += bb;
          e.bbHands += 1;
        }
    ...
    const vsOpponents: VsOpponentSummary[] = Array.from(oppMap.entries())
      .map(([opponentId, e]) => ({
        opponentId,
...
export const playerStatsService = new PlayerStatsService(gameHistoryService);
