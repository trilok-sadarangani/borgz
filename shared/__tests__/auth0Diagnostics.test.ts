import { explainAuth0Failure, validateAuth0Config } from '../auth0Diagnostics';

describe('shared/auth0Diagnostics', () => {
  describe('validateAuth0Config', () => {
    it('reports missing fields', () => {
      const issues = validateAuth0Config({}, { redirectUri: '' });
  ...
  describe('explainAuth0Failure', () => {
    it('explains callback URL mismatch with actionable steps', () => {
      const msg = explainAuth0Failure(
        ...
        { domain: 'example.auth0.com', clientId: 'abc' },
    ...
    it('explains "Service not found" as an audience/API Identifier misconfiguration', () => {
      const msg = explainAuth0Failure(
        ...
        { domain: 'example.auth0.com', clientId: 'abc', audience: 'borgz.com' },
        { redirectUri: 'http://localhost:8081' }
      );
    ...
    it('returns null for non-error responses', () => {
      const msg = explainAuth0Failure({ type: 'success', params: {} }, { domain: 'x', clientId: 'y' }, { redirectUri: 'z' });
      expect(msg).toBeNull();
    });
  });
});
